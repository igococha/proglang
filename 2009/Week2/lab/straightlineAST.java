...
class CompoundStm extends Stm {
   ...
   CompoundStm(Stm s1, Stm s2) {stm1=s1; stm2=s2;}
    ...
    int maxargs() { return Math.max(stm1.maxargs(), stm2.maxargs());  }
}
...
class AssignStm extends Stm {
   ...
   AssignStm(String i, Exp e) {id=i; exp=e;}
    ...
    int maxargs() { return exp.maxargs(); }
}
...
class PrintStm extends Stm {
   ...
   PrintStm(ExpList e) {exps=e;}
    ...
    int maxargs() { return Math.max(exps.maxargs(), exps.length()); }
}
...
class OpExp extends Exp {
    ...
    int maxargs() { return Math.max(left.maxargs(), right.maxargs()); }
}
...
class EseqExp extends Exp {
   ...
   EseqExp(Stm s, Exp e) {stm=s; exp=e;}
    ...
    int maxargs() { return Math.max(stm.maxargs(), exp.maxargs()); }
}
...
class PairExpList extends ExpList {
   ...
   public PairExpList(Exp h, ExpList t) {head=h; tail=t;}
    ...
    int maxargs() { return Math.max(head.maxargs(), tail.maxargs()); }
...
class LastExpList extends ExpList {
 ...
 public LastExpList(Exp h) {head=h;}
    ...
    int maxargs() { return head.maxargs(); }
